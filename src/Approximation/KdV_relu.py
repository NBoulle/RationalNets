...
Run the following command to use a ReLU activation function
python3 KdV_relu.py
...
class DeepHPM:
    ...
    def save_loss(self):
        ...
        np.savetxt("Results/ReLU/loss_relu.csv", L, delimiter=',')
        
        # Validation loss
        L = np.vstack((its, self.ValArray)).transpose()
        np.savetxt("Results/ReLU/val_relu.csv", L, delimiter=',')
...
if __name__ == "__main__": 
    ...
    np.savetxt("Results/ReLU/domain.csv", X_idn_star, delimiter=',')
    np.savetxt("Results/ReLU/exact_sol.csv", Exact_idn, delimiter=',')
    ...
    np.savetxt("Results/ReLU/idn_relu.csv", U_pred, delimiter=',')
    ...
    savefig('Results/ReLU/KdV_idn_relu')
